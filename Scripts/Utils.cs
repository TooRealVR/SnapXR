 ...
 * Additional code by Helge Backhaus.
 */
...
{
    [Serializable]
    public class SnapXRStoppedEvent : UnityEvent { }
    [Serializable]
    public class SnapXRSaveProgressEvent : UnityEvent<int, float> { }
    [Serializable]
    public class SnapXRSavedEvent : UnityEvent<int, string> { }
    ...
    public class ReflectionUtils<T> where T : class, new()
    {
        ...
        public void ConstrainRange<U>(Expression<Func<T, U>> fieldAccess, float value)
        {
            ...
            fieldInfo.SetValue(instance, Mathf.Clamp(value, attr.min, attr.max));
        }
        ...
        public void ConstrainRange<U>(Expression<Func<T, U>> fieldAccess, int value)
        {
            FieldInfo fieldInfo = GetField(GetFieldName(fieldAccess));
            RangeAttribute attr = GetAttribute<RangeAttribute>(fieldInfo);
            fieldInfo.SetValue(instance, (int)Mathf.Clamp(value, attr.min, attr.max));
        }
    }
}
